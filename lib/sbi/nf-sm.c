    ...
    case OGS_EVENT_SBI_CLIENT:
        ...
        CASE(OGS_SBI_SERVICE_NAME_NNRF_OAUTH2)
            if (message->AccessTokenResponse->scope && message->AccessTokenResponse->access_token) {
                if (ogs_sbi_token_add_or_update(
                    message->AccessTokenResponse->scope, 
                    message->AccessTokenResponse->access_token, 
                    OpenAPI_access_token_rsp_token_type_ToString(
                        message->AccessTokenResponse->token_type
                    )) == NULL) {
                    ogs_error("[%s] Invalid AccessTokenResponse",
                        NF_INSTANCE_ID(ogs_sbi_self()->nf_instance)
                    );
                } else {
    ...
    case OGS_EVENT_SBI_TIMER:
        ...
        case OGS_TIMER_TOKEN_FETCH_INTERVAL:
            if (ogs_sbi_self()->oauth2_enabled) {
                ...
                if (ogs_sbi_self()->token_refresh)
                    ogs_timer_start(nf_instance->t_token, ogs_time_from_sec(ogs_sbi_self()->token_refresh));
                else
                    ogs_timer_start(nf_instance->t_token, ogs_time_from_sec(1800));
            } else {
                ogs_error("oauth2 is disabled but the oauth2 timer is running");
            }
            break;
